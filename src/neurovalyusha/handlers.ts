...
async function generateValyushaText(env: NeuroValyushaBindings, messages: OpenAIChatMessage[]): Promise<string> {
...
export async function processVkCallbackEvent(env: NeuroValyushaBindings, payload: VkCallbackPayload): Promise<void> {
  ...
  if (type === 'wall_post_new') {
    ...
    const comment = normalizeOutgoingText(await generateValyushaText(env, aiMessages), 1200)
  ...
  if (type === 'wall_reply_new') {
    ...
    const reply = normalizeOutgoingText(await generateValyushaText(env, aiMessages), 1200)
